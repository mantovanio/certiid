-- Agenda online V2
-- Objetivo:
-- 1. permitir compra agora e agendamento depois
-- 2. vincular agentes permitidos por tabela de preço
-- 3. permitir agenda editável pelo agente e visão total do admin

-- ================================================================
-- BLOCO 1: RELACIONAMENTO N:N TABELA DE PREÇO X AGENTE
-- ================================================================
...
-- ================================================================
-- BLOCO 2: DISPONIBILIDADE DO AGENTE
-- ================================================================
...
-- ================================================================
-- BLOCO 3: AJUSTES PARA AGENDAMENTO PENDENTE
-- ================================================================
...
COMMENT ON COLUMN public.agendamentos_validacao.titular_id IS
  'Pode permanecer nulo quando a venda foi concluída mas o cliente ainda não escolheu o agendamento.';

COMMENT ON COLUMN public.agendamentos_validacao.agente_registro_id IS
  'Pode permanecer nulo enquanto o cliente ainda não escolheu o agente no pós-compra.';

COMMENT ON COLUMN public.agendamentos_validacao.ponto_atendimento_id IS
  'Pode permanecer nulo enquanto o cliente ainda não escolheu o ponto de atendimento.';
...
-- ================================================================
-- BLOCO 4.1: PARCEIRO / VENDEDOR / CONTADOR X AGENTE PERMITIDO
-- ================================================================
  ...
  FOR SELECT USING (
    EXISTS (
      ...
      WHERE id = auth.uid()
        ...
        AND (
          perfil = 'admin'
          OR auth.uid() = agente_registro_id
        )
    )
  );
...
-- ================================================================
-- BLOCO 6: AJUSTE DE POLÍTICAS DA AGENDA V2
-- ================================================================
  ...
  FOR SELECT USING (
    EXISTS (
      ...
      WHERE id = auth.uid()
        ...
        AND (
          perfil = 'admin'
          OR agente_registro_id = auth.uid()
          OR (perfil = 'vendedor' AND agente_registro_id IS NULL)
        )
    )
  );
